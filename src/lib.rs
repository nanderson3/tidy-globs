//! Rewrites glob patterns into a canonical form without changing what they
//! match: doubled slashes collapse, redundant `./` segments drop out,
//! repeated `**` collapses to one, and brace-group spacing gets trimmed.
//! See [`normalize`] for the single-pattern API, or [`run`] to drive the
//! same logic over a batch of patterns the way the `tidy-globs` binary does.
...
pub fn parse_args(args: &[String]) -> (Mode, Vec<String>) {
